...
test("Should use default parser options", function (t) {
  ...
  t.equal(
    ...
    "module",
    "Using default source type"
  );
  t.end();
});

const defaultEnv = ["browser", "node", "es6"];
...
const rules = [
  ...
  { name: "@typescript-eslint/no-non-null-assertion", value: "off" },
  {
    ...
    value: ["error", { argsIgnorePattern: "^_" }],
  },
...
test("Should use default rules", function (t) {
  rules.forEach((item) => {
    t.deepEqual(
      config.rules[item.name],
      item.value,
      `Rule ${item.name} has the correct value: ${item.value}`
    );
  });
  t.end();
});
